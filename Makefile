...
lib: _build_lib
	xcodebuild -project lib/$(NAME).xcodeproj -configuration Release -target $(LIB_IOS) | ${xb-prettifier}

_build_lib: _djinni
	PYTHONPATH=vendors/gyp/pylib vendors/gyp/gyp -DOS=lib --depth=. -f xcode --generator-output ./lib -Icommon.gypi $(NAME).gyp

test: _djinni
	xcodebuild -project lib/$(NAME).xcodeproj -configuration Debug TEST_MODE=1 -target test  | ${xb-prettifier} && ./build/Debug/test
