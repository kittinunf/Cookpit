...
class SearchFragment : BaseFragment() {
    ...
    override fun setUp(view: View) {
        ...
        searchTexts.filter(String::isEmpty).subscribe { controller.fetchRecents() }.addTo(subscriptions)

        searchTexts.map(String::isEmpty).map { if (it) View.VISIBLE else View.GONE }.bindTo(recentSearchRecyclerView.rx_visibility).addTo(subscriptions)
        searchTexts.map(String::isEmpty).not().map { if (it) View.VISIBLE else View.GONE }.bindTo(searchResultRecyclerView.rx_visibility).addTo(subscriptions)
        searchTexts.filter(String::isNotEmpty)
        ...
        val resetSearchItemsCommand = searchTexts.filter(String::isNotEmpty).map { SearchViewModelCommand.SetSearchItems(emptyList()) }

        val viewModels = Observable.merge(loadSearchResultsCommand, loadRecentItemsCommand, resetSearchItemsCommand)
                .scan(SearchViewModel(), SearchViewModel::executeCommand)
                .replay(1)
                .refCount()
